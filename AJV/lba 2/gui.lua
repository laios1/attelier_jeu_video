...
function bouton_rond(x, y, r, btype)
  monbouton = {}
  
  monbouton.x = x
  monbouton.y = y
  monbouton.r = r
  monbouton.type = btype or "push"
  monbouton.color = red
  if monbouton.type == toggle then
    monbouton.status = off
  end
  
  monbouton.mouseOver = function()
    if ((monbouton.x - love.mouse.getX())^2 + (monbouton.y - love.mouse.getY())^2)^0.5 <= r then
      return true
    else 
      return false 
    end
  ...
  monbouton.update = function(dt)
    
    if monbouton.type == toggle then
      if monbouton.mouseOver() then
        if love.mouse.isDown(1) then
          if monbouton.status == off then
            monbouton.status = on
          else
            monbouton.status = off
          end
  ...
  monbouton.draw = function()
    love.graphics.setColor(monbouton.color)
    love.graphics.circle("fill", monbouton.x, monbouton.y, monbouton.r)
  end
end
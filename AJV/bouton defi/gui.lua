...
function menu(x,y,w,h,listeDeFunction)
  local menu = {}
  manue.liste = listeDeFunction
  menu.x = x
  menu.y = y
  menu.w = w
  menu.h = h
  ...
  function menu.mouseover()
    if love.mouse.getX() > menu.x and love.mouse.getX() < menu.x + menu.w and love.mouse.getY() > menu.y and love.mouse.getY() < menu.y + menu.h then
      return true
    end
    return false
  end
  ...
  function menu.draw()
    ...
    if menu.isOpen then
      ...
      for i = 1,#listeDeBoutons do
        love.graphics.rectangle("fill",menu.x,menu.y+(menu.h*i)+,menu.w,menu.h)
      end
    end
  end
end
